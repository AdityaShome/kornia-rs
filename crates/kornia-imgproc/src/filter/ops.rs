...
pub fn box_blur<const C: usize, A1: ImageAllocator, A2: ImageAllocator>(
    ...
    separable_filter(src, dst, &kernel_x, &kernel_y, ExecutionStrategy::AutoRows(src.width() * C))?;
    Ok(())
}
...
pub fn sobel<const C: usize, A1: ImageAllocator, A2: ImageAllocator>(
    ...
    separable_filter(src, &mut gx, &kernel_x, &kernel_y, ExecutionStrategy::AutoRows(src.width() * C))?;

    let mut gy = Image::<f32, C, _>::from_size_val(src.size(), 0.0, CpuAllocator)?;
    separable_filter(src, &mut gy, &kernel_y, &kernel_x, ExecutionStrategy::AutoRows(src.width() * C))?;
