...
/// # Errors
///
/// Returns `ImageError::InvalidKernelSize` if `kernel_size` is even or less than 3.
pub fn sobel_kernel_1d(kernel_size: usize) -> Result<(Vec<f32>, Vec<f32>), ImageError> {
    let (kernel_x, kernel_y) = match kernel_size {
        ...
        _ => return Err(ImageError::InvalidKernelSize(kernel_size)),
    };
...
mod tests {
    ...
    fn test_sobel_kernel_1d_even_size() {
        ...
        assert!(sobel_kernel_1d(4).is_err());
    }

    #[test]
    fn test_sobel_kernel_1d_too_small() {
        // Size < 3 should return error
        assert!(sobel_kernel_1d(1).is_err());
    }
